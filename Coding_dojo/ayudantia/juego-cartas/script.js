...
const repartoUnaCarta = () => {
    let miPrimerPersonaje = misPersonajes[0]
    let miPrimerPersonajeNombre = misPersonajes[0].name
    let miPrimerPersonajeRating = misPersonajes[0].rating
    let miPrimerPersonajeImg = misPersonajes[0].img

    tablero.innerHTML = `<div> <img src=${miPrimerPersonajeImg} alt=""> <p>${miPrimerPersonajeNombre}</p> <p>${miPrimerPersonajeRating}</p> </div>`
}

repartir.addEventListener('click', repartoUnaCarta)

/* Paso:1 Plantilla para crear objetos (personajes) */
...
/* Recorrer la lista y guardar cada item en el personaje iterador (creando un objeto llamado personajeAuxObjeto) para poder agregarle propiedas y metodos */

/* Paso 2:creamos la lista donde guardaremos los objetos creados por el forEach */
const misPersonajesObjeto = []

/* Paso 3: Notación flecha */
misPersonajes.forEach(personajeIterador => {
    /* se crea personajeAuxObjeto 10 veces (una por cada personajeIterador) */
    const personajeAuxObjeto = new  CartaPersonaje(personajeIterador.name, personajeIterador.rating, personajeIterador.img)

    /* guardamos cada objeto creado por la iteración en el array */
    misPersonajesObjeto.push(personajeAuxObjeto)

}) 



/* Paso 3: Notación antigua */
